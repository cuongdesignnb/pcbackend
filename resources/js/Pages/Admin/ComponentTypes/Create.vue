...
const form = useForm({ name: '', slug: '', display_order: 0, is_required: false });
function genSlug() { form.slug = form.name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/đ/g,'d').replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,''); }
function submit() { form.post('/admin/component-types'); }
</script>
<template>
<AdminLayout title="Thêm loại linh kiện">
    <div class="max-w-lg">
        <div class="flex items-center justify-between mb-6"><h3 class="text-lg font-semibold text-slate-200">Thêm loại linh kiện</h3><Link href="/admin/component-types" class="text-sm text-slate-400 hover:text-slate-300">← Quay lại</Link></div>
