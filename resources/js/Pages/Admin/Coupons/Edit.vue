...
<AdminLayout title="Sửa mã giảm giá">
    <div class="max-w-2xl">
        <div class="flex items-center justify-between mb-6"><h3 class="text-lg font-semibold text-slate-200">Sửa: {{ coupon.code }}</h3><Link href="/admin/coupons" class="text-sm text-slate-400 hover:text-slate-300">← Quay lại</Link></div>
        <form @submit.prevent="submit" class="bg-slate-900 rounded-lg shadow-none border border-slate-800/60 p-6 space-y-4">
            <div class="grid grid-cols-2 gap-4">
                <div><label class="block text-sm font-medium text-slate-300 mb-1">Mã *</label><input v-model="form.code" class="w-full border border-slate-700/50 rounded-lg px-3 py-2 text-sm uppercase"><div v-if="form.errors.code" class="text-red-400 text-xs mt-1">{{ form.errors.code }}</div></div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div><label class="block text-sm font-medium text-slate-300 mb-1">Giá trị *</label><input v-model="form.value" type="number" class="w-full border border-slate-700/50 rounded-lg px-3 py-2 text-sm"></div>
