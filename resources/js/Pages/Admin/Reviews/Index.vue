...
<AdminLayout title="Đánh giá">
    ...
    <div class="flex justify-between items-center mb-4">
        <div class="flex items-center gap-3">
            <input v-model="search" placeholder="Tìm sản phẩm..." class="border border-slate-700/50 rounded-lg px-3 py-2 text-sm w-48">
            <select v-model="filterApproved" class="border border-slate-700/50 rounded-lg px-3 py-2 text-sm">
                <option value="">Tất cả</option><option value="1">Đã duyệt</option><option value="0">Chờ duyệt</option>
            </select>
            <select v-model="filterRating" class="border border-slate-700/50 rounded-lg px-3 py-2 text-sm">
    ...
    <div class="bg-slate-900 rounded-lg shadow-none border border-slate-800/60 overflow-hidden">
        <table class="min-w-full divide-y divide-slate-800/40">
            <thead class="bg-slate-800/40"><tr>
                ...
                <th class="px-4 py-3 text-center text-xs font-medium text-slate-400 uppercase">Đánh giá</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase">Nội dung</th>
            ...
            <tbody class="divide-y divide-slate-800/40">
                <template v-for="r in reviews.data" :key="r.id">
                    <tr class="hover:bg-slate-800/40">
                        <td class="px-4 py-3 text-sm font-medium text-slate-200 max-w-[200px] truncate">{{ r.product?.name || '—' }}</td>
                        <td class="px-4 py-3 text-sm text-slate-400">{{ r.user?.name || r.guest_name || 'Khách vãng lai' }}</td>
                        <td class="px-4 py-3 text-center text-yellow-400 text-sm">{{ '★'.repeat(r.rating) }}{{ '☆'.repeat(5 - r.rating) }}</td>
                        <td class="px-4 py-3 text-sm text-slate-400 max-w-[300px]">
                            <p v-if="r.title" class="font-medium">{{ r.title }}</p>
                            <p class="truncate">{{ r.body }}</p>
                            <p v-if="r.admin_reply" class="text-xs text-cyan-500 mt-1 italic">Đã trả lời: {{ r.admin_reply }}</p>
                        </td>
                        <td class="px-4 py-3 text-center"><span :class="r.is_approved ? 'bg-green-100 text-emerald-400' : 'bg-yellow-100 text-yellow-700'" class="px-2 py-0.5 rounded-full text-xs font-medium">{{ r.is_approved ? 'Đã duyệt' : 'Chờ duyệt' }}</span></td>
                        <td class="px-4 py-3 text-sm text-slate-400">{{ formatDate(r.created_at) }}</td>
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <button @click="replyingTo = replyingTo === r.id ? null : r.id; replyText = r.admin_reply || ''" class="text-cyan-500 hover:text-indigo-800 text-xs px-1">Trả lời</button>
                    ...
                    <tr v-if="replyingTo === r.id">
                        <td colspan="7" class="px-4 py-3 bg-slate-800/40">
                            <div class="flex gap-2 max-w-xl">
                                <input v-model="replyText" placeholder="Nhập phản hồi..." class="flex-1 border border-slate-700/50 rounded-lg px-3 py-2 text-sm">
                                <button @click="submitReply(r.id)" class="px-4 py-2 bg-cyan-600 text-white rounded-lg text-sm hover:bg-cyan-700">Gửi</button>
                                <button @click="replyingTo = null" class="px-3 py-2 text-slate-400 text-sm hover:bg-slate-700 rounded-lg">Hủy</button>
                            </div>
                        </td>
                    </tr>
                </template>
                <tr v-if="!reviews.data?.length"><td colspan="7" class="px-4 py-8 text-center text-slate-500">Chưa có đánh giá</td></tr>
            </tbody>
        </table>
    </div>
    <!-- Pagination -->
    <div v-if="reviews.last_page > 1" class="mt-4 flex justify-center gap-1">
        <template v-for="link in reviews.links" :key="link.label">
            <button v-if="link.url" @click="router.get(link.url, {}, { preserveState: true })" :class="link.active ? 'bg-cyan-600 text-white' : 'bg-slate-900 text-slate-300 hover:bg-slate-800/40'" class="px-3 py-1 border rounded text-sm" v-html="link.label"></button>
            <span v-else class="px-3 py-1 text-sm text-slate-500" v-html="link.label"></span>
        </template>
    </div>
</AdminLayout>
</template>
