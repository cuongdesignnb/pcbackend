...
const props = defineProps({ componentTypes: Array });
const form = useForm({ source_type_id: '', target_type_id: '', source_spec_key: '', target_spec_key: '', rule_type: 'must_match', allowed_values: [], power_headroom: '', message: '', is_active: true });
const ruleTypes = [
...
function submit() { form.post('/admin/compatibility'); }
</script>
<template>
<AdminLayout title="Thêm quy tắc tương thích">
    <div class="max-w-2xl">
        <div class="flex items-center justify-between mb-6"><h3 class="text-lg font-semibold text-slate-200">Thêm quy tắc</h3><Link href="/admin/compatibility" class="text-sm text-slate-400 hover:text-slate-300">← Quay lại</Link></div>
