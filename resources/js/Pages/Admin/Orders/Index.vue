...
const payColor = { pending: 'bg-yellow-100 text-yellow-700', paid: 'bg-green-100 text-emerald-400', failed: 'bg-red-100 text-red-700', refunded: 'bg-slate-800/60 text-slate-400' };
</script>
<template>
<AdminLayout title="Đơn hàng">
    ...
    <div class="bg-slate-900 rounded-lg shadow-none border border-slate-800/60 p-4 mb-4">
        <div class="flex flex-wrap items-center gap-3">
            <input v-model="search" placeholder="Mã đơn, tên KH, SĐT..." class="border border-slate-700/50 rounded-lg px-3 py-2 text-sm w-56 focus:ring-2 focus:ring-cyan-500/50">
            ...
            <select v-model="paymentStatus" @change="applyFilters()" class="border border-slate-700/50 rounded-lg px-3 py-2 text-sm">
    ...
    <div class="bg-slate-900 rounded-lg shadow-none border border-slate-800/60 overflow-hidden">
        <table class="min-w-full divide-y divide-slate-800/40">
            <thead class="bg-slate-800/40"><tr>
                <th class="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase">Mã đơn</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase">Khách hàng</th>
                <th class="px-4 py-3 text-right text-xs font-medium text-slate-400 uppercase">Tổng tiền</th>
                <th class="px-4 py-3 text-center text-xs font-medium text-slate-400 uppercase">Trạng thái</th>
                <th class="px-4 py-3 text-center text-xs font-medium text-slate-400 uppercase">Thanh toán</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase">Ngày đặt</th>
                <th class="px-4 py-3 text-right text-xs font-medium text-slate-400 uppercase">Thao tác</th>
            </tr></thead>
            <tbody class="divide-y divide-slate-800/40">
                <tr v-for="o in orders.data" :key="o.id" class="hover:bg-slate-800/40">
                    <td class="px-4 py-3 text-sm font-medium text-cyan-500">{{ o.order_number || o.order_code }}</td>
                    <td class="px-4 py-3 text-sm text-slate-200">{{ o.shipping_name || o.customer_name || o.user?.name || '—' }}</td>
                    <td class="px-4 py-3 text-sm text-slate-200 text-right font-medium">{{ formatPrice(o.total) }}</td>
                    <td class="px-4 py-3 text-center"><span :class="statusColor[o.order_status || o.status] || 'bg-slate-800/60 text-slate-400'" class="px-2 py-0.5 rounded-full text-xs font-medium">{{ statusLabel[o.order_status || o.status] || o.order_status || o.status }}</span></td>
                    <td class="px-4 py-3 text-center"><span :class="payColor[o.payment_status] || 'bg-slate-800/60 text-slate-400'" class="px-2 py-0.5 rounded-full text-xs font-medium">{{ payLabel[o.payment_status] || o.payment_status }}</span></td>
                    <td class="px-4 py-3 text-sm text-slate-400">{{ formatDate(o.created_at) }}</td>
                    <td class="px-4 py-3 text-right">
                        <Link :href="`/admin/orders/${o.id}`" class="text-cyan-500 hover:text-indigo-800 text-sm">Chi tiết</Link>
                    </td>
                </tr>
                <tr v-if="!orders.data?.length"><td colspan="7" class="px-4 py-8 text-center text-slate-500">Chưa có đơn hàng</td></tr>
            </tbody>
        </table>
    </div>
