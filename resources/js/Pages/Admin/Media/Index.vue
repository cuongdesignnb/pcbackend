...
<template>
    <AdminLayout title="Media Library">
        <!-- Flash Message -->
        <div v-if="flash?.success" class="mb-4 p-3 bg-green-50 border border-green-200 text-green-700 rounded-lg text-sm">
            {{ flash.success }}
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    <!-- Upload button -->
                    <button @click="triggerFileInput" class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors text-sm font-medium">
                    ...
                    <!-- New Folder -->
                    <button @click="showNewFolderModal = true" class="inline-flex items-center gap-2 px-3 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors text-sm">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative">
                        ...
                        <input v-model="search" type="text" placeholder="Tìm kiếm..."
                            class="pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 w-56">
                    </div>

                    <!-- Type filter -->
                    <select v-model="filterType" @change="applyFilters()" class="border border-gray-300 rounded-lg text-sm px-3 py-2 focus:ring-2 focus:ring-indigo-500">
                    ...
                    <div class="flex border border-gray-300 rounded-lg overflow-hidden">
                        <button @click="viewMode = 'grid'" :class="viewMode === 'grid' ? 'bg-indigo-50 text-indigo-600' : 'text-gray-500 hover:bg-gray-50'" class="p-2 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>
                        </button>
                        <button @click="viewMode = 'list'" :class="viewMode === 'list' ? 'bg-indigo-50 text-indigo-600' : 'text-gray-500 hover:bg-gray-50'" class="p-2 transition-colors">
        ...
        <div class="flex gap-4">
            <!-- Folder Sidebar -->
            <div class="w-48 flex-shrink-0 bg-white rounded-lg shadow-sm border border-gray-200 p-3 self-start">
                <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Thư mục</h3>
                <button @click="setFolder('/')"
                    :class="currentFolder === '/' ? 'bg-indigo-50 text-indigo-600 font-medium' : 'text-gray-700 hover:bg-gray-50'"
                ...
                <button v-for="f in folders" :key="f" @click="setFolder(f)"
                    :class="currentFolder === f ? 'bg-indigo-50 text-indigo-600 font-medium' : 'text-gray-700 hover:bg-gray-50'"
            ...
            <div class="flex-1">
                ...
                <div
                    ...
                    @drop="onDrop"
                    :class="dragOver ? 'border-indigo-400 bg-indigo-50' : 'border-transparent'"
                    class="border-2 border-dashed rounded-xl transition-colors min-h-[400px]"
                >
                    <!-- Empty State -->
                    <div v-if="!mediaItems.length" class="flex flex-col items-center justify-center py-20 text-gray-400">
                        ...
                        <button @click="triggerFileInput" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm font-medium">Upload file</button>
                    </div>
                    ...
                    <div v-else-if="viewMode === 'grid'" class="grid grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-3 p-2">
                        <div v-for="item in mediaItems" :key="item.id"
                            ...
                            :class="isSelected(item) ? 'ring-2 ring-indigo-500 bg-indigo-50' : 'hover:bg-gray-50'"
                            class="group relative rounded-lg border border-gray-200 overflow-hidden cursor-pointer transition-all"
                        >
                            <!-- Checkbox -->
                            <div class="absolute top-2 left-2 z-10">
                                <div :class="isSelected(item) ? 'bg-indigo-600 border-indigo-600' : 'bg-white/80 border-gray-300 opacity-0 group-hover:opacity-100'"
                            ...
                            <div class="aspect-square bg-gray-100 flex items-center justify-center overflow-hidden">
                                ...
                                <div v-else class="text-center p-2">
                                    <svg class="w-10 h-10 text-gray-400 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                    <span class="text-[10px] text-gray-400 uppercase font-medium">{{ item.file_name.split('.').pop() }}</span>
                                </div>
                            </div>

                            <!-- Name -->
                            <div class="px-2 py-1.5">
                                <p class="text-xs text-gray-700 truncate font-medium">{{ item.name }}</p>
                                <p class="text-[10px] text-gray-400">{{ item.human_size }}</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="w-10 px-4 py-3">
                                        <input type="checkbox" @change="selectAll" :checked="selectedItems.length === mediaItems.length && mediaItems.length > 0" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                    </th>
                                    <th class="w-12"></th>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Kích thước</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Kích cỡ</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Ngày upload</th>
                                    <th class="w-20"></th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200">
                                <tr v-for="item in mediaItems" :key="item.id"
                                    ...
                                    :class="isSelected(item) ? 'bg-indigo-50' : 'hover:bg-gray-50'"
                                    class="cursor-pointer transition-colors">
                                    <td class="px-4 py-2">
                                        <input type="checkbox" :checked="isSelected(item)" @click.stop class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                    </td>
                                    <td class="py-2">
                                    ...
                                    <td class="px-4 py-2 text-sm text-gray-500 uppercase">{{ item.file_name.split('.').pop() }}</td>
                                    <td class="px-4 py-2 text-sm text-gray-500">{{ item.human_size }}</td>
                                    <td class="px-4 py-2 text-sm text-gray-500">
                                        <span v-if="item.width">{{ item.width }}×{{ item.height }}</span>
                                        <span v-else>—</span>
                                    </td>
                                    <td class="px-4 py-2 text-sm text-gray-500">{{ formatDate(item.created_at) }}</td>
                                    <td class="px-4 py-2 text-right">
                                        <button @click.stop="openDetail(item)" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">Chi tiết</button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                ...
                <div v-if="media?.last_page > 1" class="mt-4 flex items-center justify-between">
                    <p class="text-sm text-gray-500">{{ totalItems }} file tổng cộng</p>
                    <div class="flex gap-1">
                        <button v-for="link in media.links" :key="link.label"
                            ...
                            class="px-3 py-1.5 text-sm border border-gray-300 rounded transition-colors disabled:opacity-40"
                            v-html="link.label"
                        />
                    </div>
                </div>
                ...
                <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 p-6">
                    ...
                    <div class="max-h-60 overflow-y-auto space-y-2 mb-4">
                        <div v-for="(file, fi) in uploadFiles" :key="fi"
                            ...
                            <button @click="removeUploadFile(fi)" class="text-gray-400 hover:text-red-500 p-1">
                    ...
                    <!-- Add more -->
                    <button @click="triggerFileInput" class="w-full border-2 border-dashed border-gray-300 rounded-lg p-3 text-sm text-gray-500 hover:border-indigo-400 hover:text-indigo-500 transition-colors mb-4">
                        + Thêm file
                    </button>
                    ...
                    <div class="flex justify-end gap-3">
                        <button @click="showUploadModal = false; uploadFiles = []" class="px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">Huỷ</button>
            ...
            <div v-if="showDetailPanel && detailItem" class="fixed inset-0 z-[60] flex justify-end">
                ...
                <div class="relative bg-white w-full max-w-md shadow-2xl overflow-y-auto">
                    ...
                    <div class="p-6 space-y-6">
                        <!-- Preview -->
                        <div class="bg-gray-100 rounded-xl overflow-hidden flex items-center justify-center min-h-[200px]">
                            <img v-if="isImage(detailItem)" :src="detailItem.url" :alt="detailItem.alt" class="max-w-full max-h-80 object-contain">
                            <div v-else class="text-center p-8">
                                <svg class="w-16 h-16 text-gray-400 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                <p class="text-sm text-gray-500">{{ detailItem.file_name }}</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3 text-sm">
                            <div class="flex justify-between py-1.5 border-b border-gray-100">
                                <span class="text-gray-500">File gốc</span>
                                <span class="text-gray-900 font-medium">{{ detailItem.file_name }}</span>
                            </div>
                            <div class="flex justify-between py-1.5 border-b border-gray-100">
                                <span class="text-gray-500">Loại</span>
                                <span class="text-gray-900">{{ detailItem.mime_type }}</span>
                            </div>
                            <div class="flex justify-between py-1.5 border-b border-gray-100">
                                <span class="text-gray-500">Kích thước</span>
                                <span class="text-gray-900">{{ detailItem.human_size }}</span>
                            </div>
                            <div v-if="detailItem.width" class="flex justify-between py-1.5 border-b border-gray-100">
                                <span class="text-gray-500">Kích cỡ ảnh</span>
                                <span class="text-gray-900">{{ detailItem.width }} × {{ detailItem.height }} px</span>
                            </div>
                            <div class="flex justify-between py-1.5 border-b border-gray-100">
                                <span class="text-gray-500">Ngày upload</span>
                                <span class="text-gray-900">{{ formatDate(detailItem.created_at) }}</span>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex gap-2">
                                <input :value="detailItem.url" readonly class="flex-1 text-sm bg-gray-50 border border-gray-300 rounded-lg px-3 py-2 text-gray-600">
                                <button @click="copyUrl(detailItem.url)" class="px-3 py-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 transition-colors text-sm" title="Copy URL">
                        ...
                        <div>
                            ...
                            <input v-model="detailItem.name" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Alt text</label>
                            <input v-model="detailItem.alt" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="Mô tả hình ảnh">
                        </div>

                        <!-- Actions -->
                        <div class="flex gap-3 pt-2">
                            <button @click="saveDetail" class="flex-1 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm font-medium transition-colors">Lưu thay đổi</button>
        ...
        <Teleport to="body">
            <div v-if="showNewFolderModal" class="fixed inset-0 z-[60] flex items-center justify-center">
                ...
                <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-sm mx-4 p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Tạo thư mục mới</h3>
                    <input v-model="newFolderName" @keydown.enter="createFolder" placeholder="Tên thư mục (a-z, 0-9, -, _)"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 mb-4">
                    <div class="flex justify-end gap-3">
                        <button @click="showNewFolderModal = false" class="px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">Huỷ</button>
                        <button @click="createFolder" :disabled="!newFolderName.trim()" class="px-4 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-colors font-medium">Tạo</button>
                    </div>
                </div>
            </div>
        </Teleport>
